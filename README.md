**Hi, I'm Amulya GJ**

 Computer Science Engineering Student  
 AI & Machine Learning Enthusiast  
 Ethical Hacking Learner  
 Web Developer  

---

## About Me
I am a Computer Science student interested in Artificial Intelligence, Web Development, and Cybersecurity.  
I enjoy building smart, secure, and user-friendly applications.  
Currently learning AI tools, ethical hacking, and full stack web development.

---

##  Skills
*Programming Languages*
- Python
- JavaScript
- HTML
- CSS

*AI & Machine Learning*
- Scikit-learn
- TensorFlow (Learning)
- OpenAI API
- LangChain (Learning)

*Web Development*
- HTML, CSS, JavaScript
- Responsive Design
- CRUD Applications

*Cybersecurity*
- Burp Suite
- Nmap
- OWASP Top 10
- DVWA
- Python Security Scripts

*UI/UX*
- Figma
- Wireframing
- Responsive UI Design

*Tools*
- Git
- GitHub
- VS Code
- Eclipse
- MySQL

---

##  Projects

###  Jarvis AI Assistant
Voice-controlled AI assistant built using Python and AI APIs that can open applications, search the web, and perform basic automation tasks.

###  AI Friend Chat Web App
Interactive AI chatbot web application with real-time chat interface, typing animation, and AI-generated responses.

###  Multi-User Blogging Platform
A web-based blogging system where multiple users can register, log in, and create, edit, delete blog posts using CRUD operations.

###  UI/UX Web Applications
Collection of responsive front-end web applications focusing on UI/UX design, layouts, animations, forms, and dashboards.

---

##  Currently Learning
- Machine Learning
- Deep Learning
- Cybersecurity & Ethical Hacking
- Full Stack Web Development
- Data Structures & Algorithms

---

##  Future Goals
- Build AI-based Final Year Project
- Contribute to Open Source Projects
- Become AI/ML Engineer or Security Engineer
- Develop useful applications for society

---

##  GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=AmulyaGJ&show_icons=true&theme=radical)

---

## Contact Me
- 📧 Email: amulyagj77@gmail.com
- 💻 GitHub: https://github.com/AmulyaGJ
- 🔗 LinkedIn: -
- 🌐 Portfolio: -

---

⭐ From Amulya GJ
